...
Release は Apple Developer Program メンバーシップ期限切れの前提で、手元の `Apple Development: Yoshinori Seino (J7MGJH76BD)` / Team ID `6WFKUJRXCU` に設定しています。Developer ID Application 証明書は使いません。Release のみ `ENABLE_HARDENED_RUNTIME=YES` です。Debug は引き続き ad-hoc の Sign to Run Locally です。
...
4. code signing requirement の不一致: このデモでは Release だけ Developer ID 固有マーカーを要求する壊れた requirement を適用し、Apple Development 署名を決定的に拒否します。`BUILD_USE_CORRECT_REQUIREMENT=1` で Team ID `6WFKUJRXCU` の正しい requirement に切り替えます。
