...
- **App Sandbox**（`com.apple.security.app-sandbox`）: アプリをファイルアクセスや通信の面で隔離する仕組みで、これも entitlement の 1 つです（Mac App Store 配布では必須）。有効にすると、他プロセスの Mach サービス名の照会（電話帳の番号照会）が原則拒否されるため、XPC 通信に直接影響します。sandboxed アプリは初回起動時に `~/Library/Containers/<Bundle ID>` という専用フォルダ（コンテナ）が作られます。検証方法は DIAGNOSIS の「8. App Sandbox による拒否」を参照してください。
- 注意: Mach サービスの番号照会自体は entitlement ではなく launchd への登録が前提です。Hardened Runtime が XPC 通信そのものを止めるわけではありません（止めるのは上記の App Sandbox です）。
...
| App Sandbox / コンテナ | アプリの隔離機構（Mach サービス照会も拒否対象）/ sandboxed アプリ専用の `~/Library/Containers/<Bundle ID>` |
