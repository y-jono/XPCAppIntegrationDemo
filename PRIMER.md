...
- **`TeamIdentifier=6WFKUJRXCU`**: **Team ID**。開発者アカウントごとの10桁 ID。証明書の中では **OU**（Organizational Unit）という欄に入っている。**公開情報**（秘密ではない）。
...
| `certificate leaf[subject.OU] = "6WFKUJRXCU"` | 末端証明書の OU（＝Team ID）が `6WFKUJRXCU` であること |
...
> **今回の「壊れた要件」の正体**: Release は `...6.1.13 exists`（Developer ID 専用マーカー）を要求しているのに、実際の署名は **Apple Development**（このマーカーを持たない）。だから条件不成立で拒否。`BUILD_USE_CORRECT_REQUIREMENT=1` にすると要件が `OU="6WFKUJRXCU"`（Team ID 一致）に変わり、Apple Development でも通る。
...
| 項目 | Debug | Release |
|---|---|---|
| 署名 | ad-hoc (Sign to Run Locally) | Apple Development (Team `6WFKUJRXCU`) |
...
- `TeamIdentifier=6WFKUJRXCU` → Team ID。
